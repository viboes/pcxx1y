...
\title{More Improvements to std::future<T> - Revision 2}
\author{}
\date{}
...
\item{2nd revision: }
\begin{itemize}
\item Rename \cpp{next} by  \cpp{bind}.
\item Rename \cpp{recover} by  \cpp{catch_error}.
\item Added \cpp{map}.
\item Added \cpp{unexpected_type<E>} template class.
\item Added the free function \cpp{make_unexpected()}.
\item Removed the free function \cpp{make_exceptional_ptr()}.
\item Rename \cpp{exceptional_ptr} by  \cpp{unexpected_type<exception_ptr>}.
\item Change the \cpp{get_exception_ptr()} member function by a free function \cpp{exception_ptr_cast()}.
\item Added the free function \cpp{unexpected_cast()}.
\end{itemize}
...
Been able to retrieve the stored exception would not only simplify the user code but also improve the efficiency of this code. The \cpp{exception_ptr_cast()} function is there for this purpose.
...
As \cpp{.then()} these functions allows to chain multiple asynchronous operations. By using \cpp{future.map}/\cpp{future.bind}/\cpp{future.recover}, creating a chain of continuations becomes straightforward and intuitive:
...
\subsection{\cpp{has_value()}  and \cpp{exception_ptr_cast()}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\subsection{\cpp{make_unexpected}}
%%%%%%%%%%%%%%%%%%%%%%
...
\add{
  ...
  // 30.6.y, template class unexpected_type  
  template <class Error>
  class unexpected_type;  
  
  // \ref{futures.make_unexpected_future},  Exceptional factories
  unexpected_type<exception_ptr> make_unexpected(std::exception_ptr);  
  unexpected_type<E> make_unexpected(E&& ex);  
  unexpected_type<exception_ptr> make_unexpected();  

  template <class R>
  exception_ptr exception_ptr_cast(future<T>&);
...
\add{
    ...
    // Constructor from exceptional_ptr 
    future(exceptional_ptr&&) noexcept;  
...
\item The continuation is called when the object's shared state is ready and has a value. 
...
\wordingNewSubSubSec{30.6.x Function template \cpp{exception_ptr_cast}}{futures.exception_ptr_cast}

%%%%%%%%%%%%%%%%
\begin{lstlisting}[xleftmargin=0pt]
exception_ptr exception_ptr_cast() const;
\end{lstlisting}
\begin{wordingPara}
...
\wordingNewSubSubSec{30.6.x Function template \cpp{make_unexpected}}{futures.make_unexpected}


%%%%%%%%%%%%%%%%
\begin{alltt}
unexpected_type<exception_ptr> make_unexpected(exception_ptr ex);
template <class E>
  unexpected_type<E> make_unexpected(E&& ex);
unexpected_type<exception_ptr> make_unexpected();
...
\begin{Effects}
The exception that is passed in to the function or the current exception if no parameter is given is moved into the returned \cpp{unexpected_type<exception_ptr>} if it 
is an rvalue. Otherwise the exception is copied into the returned \cpp{unexpected_type<exception_ptr>}. 
\end{Effects}
 
\begin{Returns}
An \cpp{unexpected_type<exception_ptr>} instance implicitly convertible to a \cpp{future<T>} 
\end{Returns}
...
Boost.Thread \cite{BOOST_THREAD} provides already the typedef \cpp{value_type}, the observers \cpp{has_value},  \cpp{get_exception_ptr}, \cpp{value_or/get_or}, and the factories \cpp{fallback_to}, \cpp{make_unexpected}. Not yet implemented, \cpp{bind} and \cpp{catch_error}.
